...
unsigned int marker,t;
  ...
  if (stream_format_v->colors_used!=0)
  {
    ...
    {
      putc(stream_format_v->palette[t]&255,out);
      putc((stream_format_v->palette[t]>>8)&255,out);
      putc((stream_format_v->palette[t]>>16)&255,out);
      putc(0,out);
    }
  }
...
//static int write_avi_header_chunk(FILE *out,struct kohn_avi_header_t *avi_header,struct kohn_stream_header_t *stream_header,struct kohn_stream_format_v_t *stream_format_v)
...
int d;
  ...
  d=abs((c1&255)-(c2&255))+abs(((c1>>8)&255)-((c2>>8)&255))+abs(((c1>>16)&255)-(
(c2>>16)&255));
...
int rle8_write_line(FILE *out, unsigned int *image, int width, struct stream_format_v_t *stream_format_v)
{
int count;
int r,t,k,out_count;
...
FILE *out;
  ...
  /* set stream header */
  strcpy(kavi->stream_header_v.data_type,"vids");
  ...
  kavi->stream_format_v.compression_type=((unsigned int)fourcc[3]<<24)+
...
*/
  ...
  if (kavi->avi_header.data_streams==2)
  {
    ...
    kavi->stream_header_a.buffer_size=audio->channels*(audio->bits/8)*audio->samples_per_second;
    ...
    kavi->stream_format_a.bytes_per_second=audio->channels*(audio->bits/8)*audio->samples_per_second;
...
{
  kavi->stream_header_v.data_rate=(int)fps;
...
long t;
  ...
  //kavi->stream_header_v.data_length=kavi->frame_count;
  //kavi->stream_header_a.data_length=kavi->frame_count;
  kavi->avi_header.number_of_frames=kavi->stream_header_v.data_length;

  t=ftell(kavi->out);
  fseek(kavi->out,12,SEEK_SET);
  // write_avi_header_chunk(kavi->out,&kavi->avi_header,&kavi->stream_header_v,&kavi->stream_format_v);
  write_avi_header_chunk(kavi);
  fseek(kavi->out,t,SEEK_SET);
  ...
  if (kavi->stream_format_v.palette!=0) free(kavi->stream_format_v.palette);
